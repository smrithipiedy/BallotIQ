...
- simpleExplanation: A simplified explanation of this concept written in plain, conversational language. MINIMUM 4 sentences. NEVER write just one sentence. Do NOT shorten the content — make it EASIER to understand by using simple words, everyday comparisons, and breaking the idea into small steps. Imagine explaining this to a curious 12-year-old who has never voted before. Length must be similar to detailedExplanation.
...
CRITICAL RULE: simpleExplanation must never be shorter than 3 sentences. If you find yourself writing one sentence, expand it with a real-world analogy and a practical example.
...
9. CRITICAL: simpleExplanation must never be a single sentence. If your simpleExplanation is shorter than 3 sentences, rewrite it.`;
}
